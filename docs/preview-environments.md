...
```
http://pr-<PR번호>.api.141.164.40.139.nip.io
```

PR #27이면 `pr-27.api.141.164.40.139.nip.io`다.
...
로그인이 필요한 화면 확인은 홈서버(`tapple.co.kr`)에서 한다.
...
```
VITE_SERVER_API_URL=http://pr-27.api.141.164.40.139.nip.io/v1
```
...
secrets/preview-*.yaml             시크릿 4종 (씰링 워크플로가 생성)
```
...
```
Actions → Seal secrets → Run workflow → target: preview
```

필요한 Actions 시크릿: `GHCR_USERNAME`, `GHCR_PAT`, `PREVIEW_GITHUB_TOKEN`.
...
- **URL 계열은 시크릿에 넣지 않는다.** `CORS_ALLOWED_ORIGINS`·`PUBLIC_*`·`*_REDIRECT_URI`는 PR마다 호스트가 달라야 하는데 시크릿 하나를 모든 프리뷰가 공유한다. ApplicationSet이 차트 `env`로 채운다 — k8s는 `env`가 `envFrom`을 덮는다
